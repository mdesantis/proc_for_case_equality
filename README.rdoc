...
<tt>case</tt> statements call the <tt>:===</tt> method, so I wrote a <tt>ProcForCaseEquality</tt> class that overrides <tt>Proc#===</tt>, letting the case statement call the proc passing the value of the case as parameter.
